...
def run_calibration():
    ...
    digipot_resistances = np.array([
        40, 50, 60, 69, 79, 88, 98, 108, 117, 127,
        137, 147, 156, 166, 176, 186, 195, 205, 215
    ])
    digipot_voltages = []
    ...
    all_resistances = np.concatenate((digipot_resistances, [10, 150]))
    all_voltages = np.concatenate((digipot_voltages, [ref_10v, ref_150v]))
    ...
    while not stop_live:
        ...
        if lock_in_value is None:
            ...
            if pad_type == 'single':
                ...
                fault_130_with_tol = fault_130 * 1.10  # +10% tolerance

                fault_plus10 = lock_in_value + 10
                fault_plus10_with_tol = fault_plus10 + 5  # +5 ohms tolerance

                fault_point = min(min(fault_130_with_tol, fault_plus10_with_tol), 154)

                if resistance_value > fault_point + 5:
                    canvas.itemconfig(canvas_image, image=pad_dual_red_img if flash else "")
                    pad_text = f"Pad: Fault > {fault_point:.1f}Ω"
                    flash = not flash
                elif resistance_value > fault_point:
                    canvas.itemconfig(canvas_image, image=pad_dual_peel_img)
